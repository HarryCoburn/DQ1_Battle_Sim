    ...
    def detach(self, observer, property_name=None):
        ...
        if property_name:
            if observer in self._observers.get(property_name, []):
                ...
                self.logger.debug(f"Observer detached from {property_name}")
            else:
